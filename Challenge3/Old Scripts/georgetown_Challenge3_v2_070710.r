########################################################################
## Function to perform NMF classification.
########################################################################
ClassifyUsingNMF <- function(inputData,
    ...
    nmfResults = nmf(as.matrix(inputData), rank = numClass,
    ...
    ##----------------------------------------------------------
    ## If desired, scale the i-th row of W by the standard
    ## deviation of the i-th column of H.
...
if ( runInBatchModeWithShellScripts ) {
    ...
    methodNMF     = "snmf/l"
    varianceNorm  = TRUE
    outSubDirName = "SNMFL_WithVarNorm"
    ...
    inputFolder  = file.path(topInputDir,dataSet)
