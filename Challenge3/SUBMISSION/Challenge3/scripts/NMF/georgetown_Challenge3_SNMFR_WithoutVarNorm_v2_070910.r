...
ClassifyUsingNMF <- function(inputData,
    ...
    ## Add a little bit of noise to those rows.
    ## Remove intermediate results to conserve memory.

    inputData = t(apply(inputData, 1, FixZeroRow, numVals=4, noiseMag=0.0001))

    ##----------------------------------------------------------
    ## KLUDGE peculiar to snmf/r: if the number of columns is
    ## less than numClass, add an extra column to the matrix.
    ## In its current implementation, the snmf/r method doesn't
    ## work if the number of columns is less than numClass.

    if ( ncol(inputData) < numClass ) {
        inputData = AddColumnOf1s(inputData)
    }
    ...
    for (i in 1:numFiles) {
        ...
        ## classifications, and save the result to an output CSV file.
            ...
            hardClass = ClassifyUsingNMF(flowCytData,
                            ...
                            normalizeHcolumns, magNoise=0.0001)
            rm(flowCytData)
...
if ( runInBatchModeWithShellScripts ) {
    ...
    if ( length(args) != 4 ) {
        ...
        print("Need input and output folders, CSV file containing number of expected populations,")
        print("and number of runs/iterations.")
