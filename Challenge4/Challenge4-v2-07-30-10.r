...
########################################################################
ProcessChallenge4DataSet <- function( inputDataDir,
    goldStandardClassificationDir, outputDir ) {
    ...
    ##------------------------------------------------------------------
    ## Compose list of input data files.

    inputFileList = sort(Sys.glob(file.path(inputDataDir,"*.csv")))
    numInputFiles = length(inputFileList )

    ##------------------------------------------------------------------
    ## Loop over input data files, cumulate into one matrix of
    ## predictor variables.

    for (i in 1:numInputFiles) {

        ##--------------------------------------------------------------
        ## Load gold standard classification data.

        inputData = read.csv(inputFileList[i])

        ##--------------------------------------------------------------
        ## Cumulate predictor variables.
        ## Remove intermediate results to conserve memory.
    ...
    ##------------------------------------------------------------------
    ## Load the test data set (note that this includes the files that
    ## were used for training/tuning, so be careful when assessing
    ## classification performance).

    predictResult = predict(rfResult, newdata = predictorVars, type = "votes")
    filePath = file.path(outputDir ,"predictResult.rda")
    save(predictResult, file = filePath)
