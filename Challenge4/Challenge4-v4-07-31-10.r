...
print(sprintf("Gold Standard File #%d\n",i))
        ##--------------------------------------------------------------
...
print("Optimizing mtry")
    tuneResult     = tuneRF( x = predictorVars, y = responseVar, trace = FALSE)
...
print("Running randomForest")
    ...
    rfResult = try ( { randomForest(x = predictorVars, y = responseVar,
                                    ntree = 1000, mtry = mtryOptim) } )
    if ( class(rfResult) == "try-error" ) {
        return(FALSE)
    }
...
print(sprintf("Predictor File #%d\n",i))
        ...
        predictResult  = predict(rfResult, newdata = predictorData, type = "votes")
        ...
        rm(predictorData,outputProbFile,outputProbPath)
        ...
        outputClassPath = file.path(outputDir,fileName)
        write.csv(classResult, file = outputClassPath)
        rm(predictResult,classResult,outputClassFile,outputClassPath)
...
if ( length(commandArgs(TRUE)) == 0 ) {
    ...
    mainOutputDir    =  "/home/jmm97/flowCAP/Challenge4/Results"
    ...
    for (i in 1:numDataSets) {
        ...
        pc4dResult = ProcessChallenge4Data(predictorDir,responseDir,outputDir,TRUE)
        if ( ! pc4dResult ) {
