...
#outputFolder = "/home/jmm97/flowCAP/csv/NDD/CSV/Output_curvHDR_062110"
    ##------------------------------------------------------
    ## Take the absolute value so that the data are non-negative.

    inputData = abs(inputData);
#makeEmptyFile(outputFolder,sprintf("%03d.abs.done",i),noProgressFiles)

    ##------------------------------------------------------
    ## Next, normalize variance of data columns.

#makeEmptyFile(outputFolder,sprintf("%03d.var.try",i),noProgressFiles)
    nCols   = ncol(inputData)
    dataSD  = sd(inputData)
    for (j in 1:nCols) {
        inputData[,j] = inputData[,j] / dataSD[j]
    }
#makeEmptyFile(outputFolder,sprintf("%03d.var.done",i),noProgressFiles)
...
#makeEmptyFile(outputFolder,sprintf("%03d.nmf.done",i),noProgressFiles)
    ...
    W = asinh(W)
#makeEmptyFile(outputFolder,sprintf("%03d.asinh.done",i),noProgressFiles)

    ##----------------------------------------------------------
    ## Compute curvHDR polygon gates.

#makeEmptyFile(outputFolder,sprintf("%03d.curvHDR.try",i),noProgressFiles)
    polygongate = curvHDRfilter(W, HDRlevel = hdrLevel)
#makeEmptyFile(outputFolder,sprintf("%03d.curvHDR.done",i),noProgressFiles)
...
#makeEmptyFile(outputFolder,sprintf("%03d.meanXY.try",i),noProgressFiles)
...
#makeEmptyFile(outputFolder,sprintf("%03d.meanXY.done",i),noProgressFiles)
    rm(polygonData,W)
...
#makeEmptyFile(outputFolder,sprintf("%03d.singleClass.done",i),noProgressFiles)
    rm(polygongate)
...
#makeEmptyFile(outputFolder,sprintf("%03d.resolveClass.done",i),noProgressFiles)
...
#makeEmptyFile(outputFolder,sprintf("%03d.finalClass.done",i),noProgressFiles)
...
# processFlowCAPDataSet("H:/DATA/Documents and Settings/Joe/Papers/GeneralStatistics/FlowCap/csv/StemCell/CSV",
#     "H:/DATA/Documents and Settings/Joe/Papers/GeneralStatistics/FlowCap/csv/StemCell/CSV/Output_062910",
